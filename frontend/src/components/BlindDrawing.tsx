        ...
        <div className="flex flex-col items-center gap-12 w-full max-w-6xl">
          <div className="grid grid-cols-2 gap-12 w-full">
            <div className="space-y-4">
              ...
              <img src={target_image} alt="Target" className="w-full rounded-3xl border-4 border-zinc-800" />
            </div>
            <div className="space-y-4">
              <p className="text-xl font-bold text-zinc-500 uppercase">Dessin de {dessinateur?.name}</p>
              <img src={drawing_base64} alt="Drawing" className="w-full rounded-3xl border-4 bg-white" />
            </div>
          </div>
          <button onClick={() => sendAction("next_drawing_round")} className="px-12 py-6 bg-white text-black rounded-3xl font-black text-2xl">MANCHE SUIVANTE</button>
        </div>
      )}
    </div>
  );
...
export const BlindDrawingPlayer = ({ gameState, role, sendAction }: Props) => {
  ...
  useEffect(() => {
    if (role === drawer_id && canvasRef.current) {
      ...
      if (ctx) { ctx.strokeStyle = "black"; ctx.lineWidth = 4; ctx.lineCap = "round"; ctx.lineJoin = "round"; }
    }
  }, [role, drawer_id]);
  ...
  if (role === guide_id) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-8 bg-zinc-900 text-white text-center">
        <h2 className="text-xl font-bold mb-8 uppercase opacity-50">Guide</h2>
        <img src={target_image} alt="Guide target" className="w-full max-w-sm rounded-3xl border-4 mb-8" />
        <p className="italic">Décris avec des formes géométriques.</p>
      </div>
    );
  }
  ...
  if (role === drawer_id) {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-4 bg-zinc-900 text-white touch-none">
        <div className="w-full max-w-md aspect-[4/3] bg-white rounded-3xl overflow-hidden mb-6">
        ...
        <div className="grid grid-cols-3 gap-2 w-full max-w-md">
          <button onClick={undo} className="p-4 bg-zinc-800 rounded-xl font-bold uppercase text-xs">Annuler</button>
          <button onClick={() => canvasRef.current?.getContext("2d")?.clearRect(0,0,800,600)} className="p-4 bg-zinc-800 rounded-xl font-bold uppercase text-xs">Effacer</button>
          <button onClick={() => sendAction("submit_drawing", { drawing: canvasRef.current?.toDataURL() })} className="p-4 bg-white text-black rounded-xl font-black uppercase text-xs shadow-xl">Terminer</button>
        </div>
      </div>
    );
  }
